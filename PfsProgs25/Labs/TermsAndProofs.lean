...
def h' : Int → Nat → Int := fun m ↦ fun n ↦ m - n
...
example : 3 ≤ 3 := Nat.le_refl 3

example : 3 ≤ 5 := Nat.le_step (Nat.le_step (Nat.le_refl 3))

example (n: Nat) : n ≤ n + 2 := Nat.le_step (Nat.le_step (Nat.le_refl n))
